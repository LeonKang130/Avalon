...
def generate_random_aggregate(u: torch.Tensor, force_specular: bool = False) -> Aggregate:
    ...
    ws, ys = evaluate_sf(num_spherical_harmonic_basis, u.device) @ sample_onb_uniform(u[:3]).squeeze(0), u[3:27].reshape(num_spherical_harmonic_basis, 6)
    ys[..., 2].add_(1.0)
    if force_specular:
        ys[..., 1].clamp_max_(0.1)
    bsdf_sh_coefficients = fit_sh_coefficients(ws, ys)
    eigenvectors = sample_onb_uniform(u[27:30]).squeeze(0)
    eigenvalues = u[30:].clamp_min(1e-6)
